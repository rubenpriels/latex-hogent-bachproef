...
Volgens \textcite{Yang2020} spelen hyperparameters een belangrijke rol bij deep learning-modellen. Hyperparameters zijn namelijk parameters die voor het trainen van een model ingesteld moeten worden. De prestaties van het model hangen af van de gekozen hyperparameters. Het probleem is echter dat de optimale hyperparameter combinatie vinden veel tijd kost aangezien er geen manier is om te weten welke hyperparameter het beste werkt voor een specifieke dataset zonder verschillende combinaties uit te testen.

Om dit probleem aan te pakken, bestaat er een methode genaamd hyperparameter tuning. Hierbij worden meerdere waarden tegelijk meegegeven aan het model. Het model probeert dan verschillende hyperparameter combinaties uit. Op het einde van het trainen behoudt het model de hyperparameter combinatie die het beste presteert op de dataset. Zo verhoogt de accuraatheid van het model significant.
...
ndom search en bayesian optimization. Volgens \textcite{Kee2025} zijn random search en bayesian optimization twee veelgebruikte methoden om de beste combinatie van hyperparameters te vinden.

Random search is een eenvoudige methode die willekeurige combinaties van hyperparameters uitprobeert waarbij de hyperparameter waarden op voorhand bepaald worden. Een groot voordeel van random search is dat het eenvoudig te gebruiken en snel te implementeren is. Daarnaast presteert random search goed wanneer er veel hyperparameter waarden zijn zonder dat het complexe algoritmes moet gebruiken.
...
Kortom, het literatuuronderzoek suggereert dat modellen zoals MLP, RNN en transformers geschikt zijn om de populariteit van points of interest te voorspellen op basis van censusdata. MLP zal worden ingezet voor simpliciteit en wanneer de dataset beperkt is, terwijl RNN gebruikt zal worden als tijdsafhankelijke verbanden ontdekken belangrijk is. Tot slot kunnen transformers ingezet worden als complexiteit niet belangrijk is en er een grote dataset beschikbaar is. Daarnaast worden finetuning technieken zoals random search en bayesian optimization gebruikt om de resultaten van deep learning modellen te optimaliseren.
...
Tot slot wordt elk model geoptimaliseerd door de beste hyperparameter combinatie van elk model te vinden met behulp van random search en bayesian optimization. Hierdoor zullen de deep learning-modellen beter presteren op nieuwe data.
